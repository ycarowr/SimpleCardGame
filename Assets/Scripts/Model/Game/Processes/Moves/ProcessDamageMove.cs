...
namespace SimpleCardGames.Battle
{
    ...
    public class ProcessDamageMove : ProcessBase
    {
        ...
        public void Execute()
        {
            ...
            var target = Game.TurnLogic.GetOpponent(source as IPlayer) as IDamageable;
            ...
            if (IsTargetDead(target))
                ProcessFinishGameStep.Execute(source as IPlayer);
        }

        private bool IsTargetDead(IDamageable target)
        {
            return (target as RuntimeCharacter).Attributes.Health == 0;
        }
